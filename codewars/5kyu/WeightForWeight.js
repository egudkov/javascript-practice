...
each month a list with the weights of members is published and each month he is the last on
the list which means he is the heaviest.

I am the one who establishes the list so I told him: "Don't worry any more, I will modify
the order of the list". It was decided to attribute a "weight" to numbers. The weight of a
...
before 99. Given a string with the weights of FFC members in normal order can you give this
...
function orderWeight(strng) {
  const sumOfDigits = numAsStr => numAsStr.split('').map(Number).reduce((sum, d) => sum + d);
  ...
  return strng.split(' ').sort(compareWeight).join(' ');
}

const input = '103 123 4444 99 2000';

console.log(orderWeight(input) === '2000 103 123 4444 99');
console.log(input === '103 123 4444 99 2000');
